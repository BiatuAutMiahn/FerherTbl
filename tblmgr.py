...
def nobinz(tbl):
    return False
    for i in ['100','150','160','161','170','163']:
        if i == tbl:
            return True
    return False
def pop_session():
...
def validate_bin(string):
    if re.match("^d[1-9]{3}o$",string) != None:
        return True
    elif re.match("^d[1-9]{4}$",string) != None:
        return True
    elif re.match("^[a-z][1-9]{2}$",string) != None:
        return True
    elif re.match("^[a-z][1-5]o$",string) != None:
        return True
    elif re.match("^aa[1-9]o$",string) != None:
        return True
    elif string=="cfm":
        return True
    else:
...
def build_templ():
                ...
                <body>
                    <div class="own-detail-wrap">
                        <div class="own-date own-detail">
                            <span>Date: </span><!--
                            --><span>'''+time.strftime('%Y.%m.%d',time.strptime(today, '%Y%m%d'))+'''</span>
                        </div>
        ...
        if 'polist' in tables[t]:
            ...
            for po in tables[t]['polist']:
                if 'rev' in tables[t]['polist'][po]:
                    for r in list(tables[t]['polist'][po]['rev'].keys()):
                        if j==cols:
                            l+=1
                            j=0
            ...
            # print(str(cols)+','+str(f)+','+str(int(cols//3)))
            if f<int(cols//2):
                l+=1
                if not l in templ[t]['lines']:
                    templ[t]['lines'][l]=[]
    ...
    for p in pages:
        ...
        spaces=layout['default']['plines']
        for t in pages[p]:
            tlines+=len(pages[p][t]['lines'])
        tpo=list(pages[p])[-1]
        nl=len(pages[p][tpo]['lines'])
        if tpo in layout:
            cols=layout[tpo]['cols']
            plines=layout[tpo]['plines']
            spaces=layout[tpo]['plines']
        while True:
            if tlines==plines:
                break
            if not nl in pages[p][tpo]['lines']:
                if len(pages[p][tpo]['lines'])>plines:
                    break
                pages[p][tpo]['lines'][nl]=[]
            nj=0
            while True:
                if nj==cols:
                    break;
                pages[p][tpo]['lines'][nl].append("")
                nj+=1
            tlines+=1
            nl+=1
...
def export_totals(shift,nobinz=False):
    # if pmpt != 'abort':
    expf="Cycles_"+shift
    ...
    new_tots_nbin={'PO':[],'Description':[],'Quantity':[],'Boxes':[],'Partials':[],'Totals':[]}
    new_tots_roll={'PO':[],'Description':[],'Last Seen':[]}
    new_tots_persist={'PO':[],'Description':[],'Total':[],'Bin':[],'First Seen':[],'Last Seen':[],'Duration':[]}
    new_tots_lownout={'PO':[],'Description':[],'Last Coverage Date':[],'Bin':[],'Quantity':[],'Boxes':[],'Partials':[],'Totals':[]}
    dates=list(totals.keys())
    dates=dates[::-1]
    # dates=dates[dates.index(date)-4:dates.index(date)+1][::-1]
    for d in dates:
        tstamp=time.strftime('%Y.%m.%d',time.strptime(d, '%Y%m%d'))
        if isinstance(totals[d],list):
            continue
        tblz_roll=OrderedDict(natsort.natsorted(totals[d].items()))
        for p in tblz_roll:
            if not '-' in p:
                continue
            desc=None
            ...
            if '???' in p:
                obs=False
                for pp in range(0,len(new_tots_roll['PO'])):
                    if p[0:-4] == p[0:-4]:
                        obs=True
                        break
                if obs==True:
                    continue
            if not p in new_tots_roll['PO']:
                new_tots_roll['PO'].append(p)
                if desc!=None:
                    new_tots_roll['Description'].append(desc)
                else:
                    new_tots_roll['Description'].append("")
                new_tots_roll['Last Seen'].append("")
    dates=list(totals.keys())
    dates=dates[dates.index(date)-4:dates.index(date)+1][::-1]
    for d in dates:
        tstamp=time.strftime('%Y.%m.%d',time.strptime(d, '%Y%m%d'))
        if not tstamp in new_tots_roll:
            new_tots_roll[tstamp]=[]
        for i in range(0,len(new_tots_roll['PO'])):
            new_tots_roll[tstamp].append("")
    # print(new_tots_roll)
    for d in dates:
        tstamp=time.strftime('%Y.%m.%d',time.strptime(d, '%Y%m%d'))
        if isinstance(totals[d],list):
        ...
        tblz_roll=OrderedDict(natsort.natsorted(totals[d].items()))
        for p in tblz_roll:
            if isinstance(tblz_roll[p],dict):# New Format with bins
                newt=0
                if 'counts' in tblz_roll[p]:
                    for c in tblz_roll[p]['counts']:
                        if '*' in c:
                            splt=c.split('*')
                            ...
                            newt+=int(c)
                newtnb=newt
                if 'ovf' in tblz_roll[p]:
                    for b in tblz_roll[p]['ovf']:
                        if len(tblz_roll[p]['ovf'][b])>0:
                            for c in tblz_roll[p]['ovf'][b]:
                                if '*' in c:
                                    splt=c.split('*')
                                    newtnb+=int(splt[0])*int(splt[1])
                                else:
                                    newtnb+=int(c)
                # print(p)
                if p in new_tots_roll['PO']:
                    new_tots_roll[tstamp][new_tots_roll['PO'].index(p)]=newtnb
    for i,p in enumerate(new_tots_roll['PO']):
        for d in list(totals.keys()):
            if p in totals[d]:
                new_tots_roll['Last Seen'][i]=time.strftime('%Y.%m.%d',time.strptime(d, '%Y%m%d'))
        ...
        if isinstance(tblz[p],dict):# New Format with bins
            ...
            for lpo in reversed(new_tots['PO']):
                if lpo=='':
                    ...
                    if lpo!=p:
                        ...
                        samepo=True
                    break
            for lpo in reversed(new_tots_lownout['PO']):
                if lpo=='':
                    continue
                else:
                    if lpo!=p:
                        samepo_lownout=False
                    else:
                        samepo_lownout=True
                    break
            ...
            if newt>0:
                ...
                if quant != None and quant>0:
                    ...
                    if (newt//quant)<=3:
                        if not samepo_lownout:
                            new_tots_lownout['PO'].append(p)
                        else:
                            new_tots_lownout['PO'].append('')
                        if tbin != None and tbin!='':
                            new_tots_lownout['Bin'].append(format_bin(tbin))
                        else:
                            new_tots_lownout['Bin'].append("")
                        if quant != None and quant>0:
                            new_tots_lownout['Quantity'].append(quant)
                            new_tots_lownout['Boxes'].append(newt//quant)
                            new_tots_lownout['Partials'].append(newt%quant)
                        else:
                            new_tots_lownout['Quantity'].append("")
                            new_tots_lownout['Boxes'].append("")
                            new_tots_lownout['Partials'].append("")
                        if desc != None and not samepo:
                            new_tots_lownout['Description'].append(desc)
                        else:
                            new_tots_lownout['Description'].append("")
                        new_tots_lownout['Totals'].append(0)
                else:
                ...
                if quant != None and quant>0:
                    ...
                    new_tots_lownout['Quantity'].append("")
                new_tots_lownout['Boxes'].append(0)
                new_tots_lownout['Partials'].append(0)
                if desc != None and not samepo:
                    new_tots_lownout['Description'].append(desc)
                else:
                    new_tots_lownout['Description'].append("")
            ...
            if newtnb>0:
                if not samepo:
                    new_tots_nbin['PO'].append(p)
                else:
                    new_tots_nbin['PO'].append('')
                if quant != None and quant>0:
                    new_tots_nbin['Quantity'].append(quant)
                    new_tots_nbin['Boxes'].append(newtnb//quant)
                    new_tots_nbin['Partials'].append(newtnb%quant)
                else:
                    new_tots_nbin['Quantity'].append("")
                    new_tots_nbin['Boxes'].append("")
                    new_tots_nbin['Partials'].append("")
                if desc != None and not samepo:
                    new_tots_nbin['Description'].append(desc)
                else:
                    new_tots_nbin['Description'].append("")
                new_tots_nbin['Totals'].append(newtnb)
            if 'ovf' in tblz[p]:
                for b in tblz[p]['ovf']:
                    if len(tblz[p]['ovf'][b])>0:
                        for lpo in reversed(new_tots['PO']):
                    ...
                    if newt>0:
                        ...
                        if quant != None and quant>0:
                            ...
                            new_tots['Boxes'].append(newt//quant)
                            new_tots['Partials'].append(newt%quant)
                            if (newt//quant)<=3 and tbin==b:
                                if not samepo_lownout:
                                    new_tots_lownout['PO'].append(p)
                                else:
                                    new_tots_lownout['PO'].append('')
                                new_tots_lownout['Bin'].append(format_bin(b))
                                if quant != None and quant>0:
                                    new_tots_lownout['Quantity'].append(quant)
                                    new_tots_lownout['Boxes'].append(newt//quant)
                                    new_tots_lownout['Partials'].append(newt%quant)
                                    ...
                                    new_tots_lownout['Partials'].append("")
                                new_tots_lownout['Totals'].append(0)
                                if desc != None and not samepo_lownout:
                                    new_tots_lownout['Description'].append(desc)
                                else:
                                    new_tots_lownout['Description'].append('')
                                new_tots_lownout['Totals'].append(0)
                        ...
                        if tbin==b:
                            ...
                            new_tots_lownout['Bin'].append(format_bin(b))
                            if quant != None and quant>0:
                                new_tots_lownout['Quantity'].append(0)
                                new_tots_lownout['Boxes'].append(0)
                                new_tots_lownout['Partials'].append(0)
                            else:
                                new_tots_lownout['Quantity'].append("")
            ...
            new_tots['Totals'].append(newt)
    # print(new_tots)
    for tt in new_tots_lownout:
        print(tt+": "+str(len(new_tots_lownout[tt])))
    print("\rFormatting Totals...done     ")
    sys.stdout.flush()
    # for tt in new_tots_nbin:
    #     print(tt+": "+str(len(new_tots_nbin[tt])))
    print("\rGenerating Spreadsheet...",end="")
    sys.stdout.flush()
    stamp=time.strftime('%Y.%m.%d',time.strptime(date, '%Y%m%d'))
    expf+='_'+stamp
    df = pd.DataFrame.from_dict(new_tots)
    dfnb = pd.DataFrame.from_dict(new_tots_nbin)
    dfroll=pd.DataFrame.from_dict(new_tots_roll)
    # dfpersist=pd.DataFrame.from_dict(new_tots_persist)
    dflownout=pd.DataFrame.from_dict(new_tots_lownout)
    ...
    dfnb.to_excel(writer, sheet_name='Cycle Counts (No Bin)',index=False)  # send df to writer
    dfroll.to_excel(writer, sheet_name='5 Day Coverage',index=False)  # send df to writer
    dfroll=dfroll.sort_values(by=['PO'])
    # dfpersist.to_excel(writer, sheet_name='Dormancies (WIP)',index=False)  # send df to writer
    dflownout.to_excel(writer, sheet_name="5 Day Low 'n Out (WIP)",index=False)  # send df to writer
    ...
    worksheet.set_header("&L"+shift+" Shift Cycle Counts "+stamp+"&RPage:&P/&N",{'margin':0.1})
    worksheet.fit_to_pages(1,0)
    worksheet.set_margins(left=0.25,right=0.25,top=0.5,bottom=0.0)

    worksheet = writer.sheets['Cycle Counts (No Bin)']
    for idx, col in enumerate(dfnb):  # loop through all columns
        series = dfnb[col]
        max_len = max((
            series.astype(str).map(len).max(),  # len of largest item
            len(str(series.name))  # len of column name/header
        ...
        worksheet.set_column(idx, idx, max_len)  # set column width
    worksheet.conditional_format('A1:'+chr(ord('A')+len(new_tots_nbin)-1)+str(len(new_tots_nbin['PO'])+1),{'type': 'formula',
                                                                              'criteria': '=MOD(ROW(),2) = 0',
                                                                              'format': formatShadeRows})
    worksheet.conditional_format('A1:'+chr(ord('A')+len(new_tots_nbin)-1)+str(len(new_tots_nbin['PO'])+1),{'type': 'no_errors','format': formatBorders})
    worksheet.freeze_panes(1,0)
    worksheet.set_header("&L"+shift+" Shift Cycle Counts (No Bin) "+stamp+"&RPage:&P/&N",{'margin':0.1})
    worksheet.fit_to_pages(1,0)
    worksheet.set_margins(left=0.25,right=0.25,top=0.5,bottom=0.0)

    worksheet = writer.sheets["5 Day Low 'n Out (WIP)"]
    for idx, col in enumerate(dflownout):  # loop through all columns
        series = dflownout[col]
        max_len = max((
            series.astype(str).map(len).max(),  # len of largest item
            len(str(series.name))  # len of column name/header
            )) + 1  # adding a little extra space
        worksheet.set_column(idx, idx, max_len)  # set column width
    worksheet.conditional_format('A1:'+chr(ord('A')+len(new_tots_lownout)-1)+str(len(new_tots_nbin['PO'])+1),{'type': 'formula',
                                                                              'criteria': '=MOD(ROW(),2) = 0',
                                                                              'format': formatShadeRows})
    worksheet.conditional_format('A1:'+chr(ord('A')+len(new_tots_lownout)-1)+str(len(new_tots_nbin['PO'])+1),{'type': 'no_errors','format': formatBorders})
    worksheet.freeze_panes(1,0)
    worksheet.set_header("&L"+shift+" Shift Low 'n Out "+stamp+"&RPage:&P/&N",{'margin':0.1})
    worksheet.fit_to_pages(1,0)
    worksheet.set_margins(left=0.25,right=0.25,top=0.5,bottom=0.0)

    worksheet = writer.sheets['5 Day Coverage']
    for idx, col in enumerate(dfroll):  # loop through all columns
        series = dfroll[col]
        max_len = max((
            series.astype(str).map(len).max(),  # len of largest item
            len(str(series.name))  # len of column name/header
    ...
    formatGreen = workbook.add_format({'border':1,
                                           'bg_color': '#00ff87',
                                           'font_color': 'black',
                                           'border_color': 'black'})
    formatYellow = workbook.add_format({'border':1,
                                           'bg_color': '#ffff5f',
                                           'font_color': 'black',
                                           'border_color': 'black'})
    formatRed = workbook.add_format({'border':1,
                                           'bg_color': '#ff5f5f',
                                           'font_color': 'black',
                                           'border_color': 'black'})
    worksheet.conditional_format('A1:'+chr(ord('A')+len(new_tots_roll)-1)+str(len(new_tots_roll['PO'])+1),{'type': 'no_errors','format': formatBorders})

    for idx in range(2,len(new_tots_roll['PO'])):  # loop through all columns
        # if idx==1:
        #     continue
        lastdt=datetime.fromtimestamp(mktime(time.strptime(new_tots_roll['Last Seen'][idx],'%Y.%m.%d')))
        tdelta=todaydt-lastdt
        # print(str(idx)+','+new_tots_roll['PO'][idx]+','+new_tots_roll['Last Seen'][idx]+','+str(tdelta.days)+',Green')
        if tdelta.days<=6:
            worksheet.conditional_format('C'+str(idx),{'type': 'no_errors','format': formatGreen})
        elif tdelta.days>=21:
            worksheet.conditional_format('C'+str(idx),{'type': 'no_errors','format': formatRed})
        elif tdelta.days>=7:
            worksheet.conditional_format('C'+str(idx),{'type': 'no_errors','format': formatYellow})

    worksheet.conditional_format('A1:A'+str(len(new_tots_roll['PO'])+1),{'type': 'formula',
                                                                              'criteria': '=MOD(ROW(),2) = 0',
                                                                              'format': formatShadeRows})
    worksheet.freeze_panes(1,0)
    worksheet.set_header("&L"+shift+" Shift 5 Day Coverage"+stamp+"&RPage:&P/&N",{'margin':0.1})
    worksheet.fit_to_pages(1,0)
    worksheet.set_margins(left=0.25,right=0.25,top=0.5,bottom=0.0)
    writer.save()
...
todaydt=None
date=None
# with open('temp.log','w+') as tmp:
if len(sys.argv)==2:
    if sys.argv[1]=='gentmpl':
        clear_screen()
        ...
        userdat = {}
        totals = {}
    elif sys.argv[1]=='prnalltmpl':
        print("Sending Print Notification...")
        task={'print':[]}
        task['print'].append("https://fehrertbl.infinitycommunicationsgateway.net/pdf/020.pdf")
        task['print'].append("https://fehrertbl.infinitycommunicationsgateway.net/pdf/050.pdf")
        task['print'].append("https://fehrertbl.infinitycommunicationsgateway.net/pdf/210.pdf")
        task['print'].append("https://fehrertbl.infinitycommunicationsgateway.net/pdf/misc.pdf")
        task['print'].append("https://fehrertbl.infinitycommunicationsgateway.net/pdf/090.pdf")
        task['print'].append("https://fehrertbl.infinitycommunicationsgateway.net/pdf/383.pdf")
        with open('pdf/notify.json','w+') as f:
            f.write(json.dumps(task))
        print("Press any key to logout...")
        wait_key()
        clear_screen()
    elif sys.argv[1]=='prn020tmpl':
        print("Sending Print Notification...")
        task={'print':[]}
        task['print'].append("https://fehrertbl.infinitycommunicationsgateway.net/pdf/020.pdf")
        with open('pdf/notify.json','w+') as f:
...
while True:
    ...
    if len(userdat['detail']['FirstName'])==0 or len(userdat['detail']['LastName'])==0 or len(userdat['detail']['Email'])==0 or len(userdat['detail']['Phone'])==0 or len(userdat['detail']['Shift'])==0:
        ...
        if len(userdat['detail']['FirstName'])==0:
            while True:
                ...
                pmpt=pmpt.translate(string.whitespace)
                if len(pmpt)>0:
                    break
    ...
    if userdat['detail']['FirstName']!="" and userdat['detail']['LastName']!="":
        print('\x1b[0;30;47m'+userdat['detail']['FirstName'].capitalize()+' '+userdat['detail']['LastName'].capitalize()+'\x1b[0m',end=" ")
    ...
    while True:
        ...
        if cmd=="help":
            ...
            print('[Help]')
            print('--Work tasks------------------------------------------------')
            print('mytables : Manage Product Tables/Enter Counts')
            print('--User Actions----------------------------------------------')
        ...
        #                 continue
            ...
            while True: # Print UI
                ...
                for key in tables:
                    hasc=False
                    keystr=key
                    lastdt=datetime.fromtimestamp(mktime(time.strptime(tables[key]['times']['accessed'],'%Y%m%d%H%M%S')))
                    tdelta=todaydt-lastdt
                    if hascounts(key):
                        hasc=True
                    ...
                    if y==7:
                        ...
                        if key==table:
                            if hasc:
                                ...
                                if tdelta.days>2:
                                    keystr=str('\x1b[48;5;203m\x1b[38;5;0m'+key+'\x1b[0m') # Red
                                elif tdelta.days>1:
                                    keystr=str('\x1b[48;5;185m\x1b[38;5;0m'+key+'\x1b[0m') # Yellow
                                else:
                                    keystr=str('\x1b[48;5;048m\x1b[38;5;0m'+key+'\x1b[0m') # Green
                                print(keystr)
                        else:
                            if tdelta.days>2:
                                keystr=str('\x1b[38;5;203m'+key+'\x1b[0m') # Red
                            elif tdelta.days>1:
                                keystr=str('\x1b[38;5;185m'+key+'\x1b[0m') # Yellow
                            else:
                                keystr='\x1b[38;5;048m'+key+'\x1b[0m' # Green
                            if hasc:
                                keystr=str('\x1b[38;5;111m'+key+'\x1b[0m') # Blue
                            print(keystr)
                    else:
                        if key==table:
                            if hasc:
                        ...
                        if len(tables[table]['polist']) > 0:
                            ...
                            for po in tables[table]['polist']:
                                ...
                                if 'rev' in tables[table]['polist'][po]:
                                    for r in list(tables[table]['polist'][po]['rev'].keys()):
                                        lastdt=datetime.fromtimestamp(mktime(time.strptime(tables[table]['polist'][po]['rev'][r]['times']['accessed'],'%Y%m%d%H%M%S')))
                                        tdelta=todaydt-lastdt
                                        postr=str(po+'-'+r)
                                        if tdelta.days>=21:
                    ...
                    if table != None and prod != None:
                        ...
                        if stable != None:
                            ...
                            if "bin" in tables[table][stable]['polist'][prod]:
                                if tables[table][stable]['polist'][prod]["bin"] != '':
                                                ...
                                                if len(usertbl[t]['polist'][p]['ovf'][b])>0:
                                                    for c in usertbl[t]['polist'][p]['ovf'][b]:
                                                        ...
                                                        if not c in totals[date][t+p]['ovf'][b]:
                                                            ...
                                                            tables[t]['polist'][splp[0]]['rev'][splp[1]]['times']['accessed']=stamp
                                                            counts+=1
                ...
                if inp[0]=='/':
                    ...
                    if table != None:
                        if userdat['role']=='admin':
                            if '-' in inp[2:]:
                                ...
                                spl=inp[2:].split('-')
                                if len(spl[1])!=3 or (not spl[1].isdigit() and not '?' in spl[1]):
                            ...
                            if not 'polist' in usertbl[table] or not 'polist' in tables[table]:
                                ...
                                if inp[2:]=="":
                                    ...
                                    for i,k in enumerate(list(tables[table]['polist'].keys())):
                                        if k==prod:
                                            if 'rev' in tables[table]['polist'][k] and rev != None:
                                                prevs=list(tables[table]['polist'][k]['rev'].keys())
                                                hasrev=False
                                                for j,l in enumerate(prevs):
                                                    if l==rev:
                                                        if len(tables[table]['polist'][k]['rev'])>j+1:
                                                            prevs=list(tables[table]['polist'][k]['rev'].keys())
                                                            inp="++"+prod+'-'+prevs[j+1]
                                                            print("++"+prod+'-'+prevs[j+1])
                                                            hasrev=True
                                                            break
                                                if not hasrev:
                                                    if len(list(tables[table]['polist'].keys()))>i+1:
                                                        ...
                                                        if 'rev' in tables[table]['polist'][tprod]:
                                                            prevs=list(tables[table]['polist'][tprod]['rev'].keys())
                                                            inp="++"+tprod+'-'+prevs[0]
                                                            rev=prevs[0]
                                                        else:
                                                            inp="++"+tprod+'-???'
                                ...
                                if not rev in tables[table]['polist'][inp[2:]]['rev']:
                                    if rev!='???':
                                        if '???' in tables[table]['polist'][inp[2:]]['rev']:
                                            tables[table]['polist'][inp[2:]]['rev'].pop('???',None)
                                    tables[table]['polist'][inp[2:]]['rev'][rev]={'times':{'created':stamp,'modified':stamp,'accessed':stamp}}
                            prod=inp[2:]
                            sync_tables()
                            if userdat['role']=='user':
                                write_usertbl()
                    ...
                    if ',' in inp[1:]:
                        ...
                        # print(inp)
                        # print(zbin)
                        if re.match("^[a-z][1-5]$",spl[0]) != None:
                            zbin+='o'
                        elif re.match("^d[1-9]{3}$",spl[0]) != None:
                            zbin+='o'
                        # print(zbin)
                        # wait_key()
                        if validate_bin(zbin):
                            for i in range(1,len(spl)):
                                if '*' in spl[i]:
                                    ...
                                    if len(spla)>2:
                                        ...
                                        time.sleep(2)
                                        continue
                                    add_count('+'+spla[0]+'*'+spla[1],zbin)
                                else:
                                    add_count('+'+spl[i],zbin)
                        elif spl[0].isdigit():
                            for i in range(1,len(spl)):
                                if '*' in spl[i]:
                                    spla=spl[i].split('*')
                                    if len(spla)>2:
                                        ...
                                        time.sleep(2)
                                        continue
                                    if re.match("^[a-z][1-5]$",spla[0]) != None:
                                        spla=str(spla[0]+'o'+'*'+spla[1]).split('*')
                                    elif re.match("^d[1-9]{3}$",spla[0]) != None:
                                        spla=str(spla[0]+'o'+'*'+spla[1]).split('*')
                                    ...
                                    if re.match("^[a-z][1-5]$",spl[i]) != None:
                                        spl[i]+='o'
                                    elif re.match("^d[1-9]{3}$",spl[i]) != None:
                                        spl[i]+='o'
